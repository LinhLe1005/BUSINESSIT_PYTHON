...
with tab2:
    ...
    col1, col2 = st.columns([3, 4])
    with col1:
       heartdisease_type = st.radio("Do you want to see the chart which has heart disease? 👇", ["HeartDisease", "Normal"], key="radio_heartdisease_type", disabled=st.session_state.disabled)
    with col2:
       rank = st.selectbox("Categories", ("ST_Slope", "ExerciseAngina", "FastingBS"), key="selectbox_rank", disabled=st.session_state.disabled)
